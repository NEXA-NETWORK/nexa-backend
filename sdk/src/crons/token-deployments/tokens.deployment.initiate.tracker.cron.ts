...
@Injectable()
export class TokensDeploymentInitiateEventTrackerCronJobService
  implements OnApplicationBootstrap
{
  private isCronRunning = false; // for cron status
  private tokenDeploymentChains: ITokenInitiateDeploymentChain[];
  constructor(
  ...
  startCronJob(): CronJob {
    ...
    return new CronJob('0 */3 * * * *', async () => {
      if (!this.isCronRunning) {
        ...
        for (const tokenDeploymentChain of this.tokenDeploymentChains) {
          const chain = tokenDeploymentChain.chainId;
          const catRelayerAddress = getCATRelayerAddress(chain);
          const symbol = `CAT-${chain}-${catRelayerAddress}-INITIATE-TOKEN-DEPLOYMENT`;
          try {
            ...
            const startBlock = blockchainInfo
              ...
              : tokenDeploymentChain.initialBlock; //
            ...
            const eventFilter =
              catRelayerContract.filters.InitiateTokenDeployment();
            ...
            console.log(
              'Events for Initiate Deployment -> ',
              events.length,
              startBlock,
              endBlock,
            );
            ...
            for (const event of events) {
              ...
              const [name, symbol, decimals, totalSupply, salt, owner] =
                decodeParameters(
                  [
                    'string',
                    'string',
              ...
              if (!tokenInfo) {
                console.log('No Token info found for salt -> ', salt);
                continue;
              }
              ...
              if (
                ...
                ) !== tokenInfo.totalGasFeeOnFeePaymentChain
                // &&
              ...
              for (const destinationChain of destinationChainInEventArgs) {
                await this.TokenNetworksModel.findOneAndUpdate(
                  {
                    tokenInfo: tokenInfo._id,
